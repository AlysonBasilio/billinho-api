...
RSpec.describe 'Students API', type: :request do
  ...
  let!(:students) { create_list(:students, 10) }
  ...
  describe 'POST /students' do
    # valid payload
    let(:valid_attributes) { { name: 'Alyson', cpf: '123456789', gender: 'M', payment_method: 'Boleto' } }

    context 'when the request is valid' do
      before { post '/students', params: valid_attributes }

      it 'creates a student' do
        expect(json['name']).to eq('Alyson')
      end
    ...
    context 'when the request is invalid' do
      before { post '/students', params: { cpf: '0125128123', gender: 'F', payment_method: 'Cartao' } }
  ...
  describe 'PUT /students/:id' do
    let(:valid_attributes) { { name: 'Jessica' } }
